"""Mistral AI translation backend."""
...
class MistralBackend(TranslationBackend):
    name = "Mistral AI"
    supports_batch = True
    supports_system_prompt = True

    def __init__(self, api_key: Optional[str], model: str = "mistral-small-latest",
    ...
    def _init_sync(self):
        with self._init_lock:
            ...
            try:
                ...
                raise RuntimeError(f"Mistral AI не установлен: {e}")
    ...
    def _init_async(self):
        with self._init_lock:
            ...
            try:
                from mistralai import Mistral
                self._aclient = Mistral(api_key=self.api_key)
            except Exception as e:
                raise RuntimeError(f"Mistral AI не установлен: {e}")
            try:
    ...
    async def _async_translate_one(self, aclient, sem, text: str, src_lang: str, dst_lang: str):
        ...
        for attempt in range(self.max_retries):
            async with sem:
                try:
                    res = await aclient.chat.complete(
                        model=self.model,
                        messages=self._messages(sys_prompt, payload),
                        temperature=self.temperature,
                        stream=False
