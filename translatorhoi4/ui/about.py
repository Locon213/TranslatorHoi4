...
class AboutDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        window_title = CaptionLabel("About TranslatorHoi4", header)
        ...
        header_layout.addWidget(window_title)
        ...
        notes_title = SubtitleLabel("Release notes", content)
        notes_title.setObjectName("sectionTitle")
        notes_hint = BodyLabel("Markdown is rendered with links.", content)
        notes_hint.setObjectName("mutedLabel")
    ...
    def _render_update_info(self, update_info: dict) -> None:
        ...
        if self._update_info.get("error"):
            self._set_status(f"Update status: check failed ({self._update_info['error']})", "#f87171")
            ...
            self._set_status(f"Update available: {version} ({asset_name})", "#22d3ee")
        elif self._update_info.get("latest_version"):
            self._set_status(f"Up to date. Latest version: {self._update_info['latest_version']}", "#34d399")
        else:
            self._set_status("Update status: not checked yet", "#94a3b8")
    ...
    def _check_updates(self, force: bool = True) -> None:
        if self._update_thread and self._update_thread.isRunning():
            return
        self._set_status("Update status: checking...", "#facc15")
    ...
    def _download_update(self) -> None:
        if self._download_thread and self._download_thread.isRunning():
            return
        self._set_status("Update status: downloading update package...", "#22d3ee")
        self.btn_download_update.setEnabled(False)
    ...
    def _on_download_finished(self, info: dict) -> None:
        ...
        self._set_status(f"Update {version}: {message}", "#34d399")
        self.btn_download_update.setEnabled(bool(self._update_info.get("download_url")))

    def _on_download_failed(self, error: str) -> None:
        self._set_status(f"Update status: failed to download update ({error})", "#f87171")
        self.btn_download_update.setEnabled(bool(self._update_info.get("download_url")))
